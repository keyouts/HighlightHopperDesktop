...
const allowedColors = new Set(["yellow", "lightgreen", "lightskyblue", "pink", "orange"])
...
function normalizeColor(color) {
  const normalized = limitString(color, 32).trim().toLowerCase()
...
function canonicalizeUrl(url) {
  ...
  try {
    ...
    u.hash = ""
    return u.toString().slice(0, maxUrlLength)
  } catch (e) {
    return ""
  }
}
...
function cloneHighlight(h, index = 0) {
  return {
    ...
    text: limitString(h && h.text, maxTextLength),
    color: normalizeColor(h && h.color),
    note: limitString(h && h.note, maxNoteLength),
...
fileInput.addEventListener("change", () => {
  ...
  if (file.size > maxImportBytes) {
    announce("CSV import is limited to 5 MB")
    return
  }
