...
function getBoardBounds(pinned) {
  ...
  let maxY = 600
  pinned.forEach(h => {
    maxX = Math.max(maxX, (Number(h.pinX) || 0) + 320)
    maxY = Math.max(maxY, (Number(h.pinY) || 0) + 220)
  })
...
function buildBoardSvg() {
  ...
  state.pinboardConnections.forEach(c => {
    ...
    parts.push('<rect x="' + (midX - 42) + '" y="' + (midY - 12) + '" width="84" height="24" fill="#fff" stroke="#000" stroke-width="2"/>')
    parts.push('<text x="' + midX + '" y="' + (midY + 5) + '" font-family="system-ui, sans-serif" font-size="12" font-weight="700" text-anchor="middle">' + escapeXml(getConnectionLabel(c)) + '</text>')
  })
  ...
  pinned.forEach(h => {
    ...
    parts.push('<g>')
    parts.push('<rect x="' + (x + 6) + '" y="' + (y + 6) + '" width="240" height="150" fill="#000"/>')
    parts.push('<rect x="' + x + '" y="' + y + '" width="240" height="150" fill="#fff" stroke="#000" stroke-width="3"/>')
    parts.push('<rect x="' + (x + 12) + '" y="' + (y + 12) + '" width="18" height="18" fill="' + escapeXml(h.color || "yellow") + '" stroke="#000" stroke-width="2"/>')
    wrapSvgText(h.text || "", 30).forEach((line, i) => {
      parts.push('<text x="' + (x + 12) + '" y="' + (y + 48 + i * 16) + '" font-family="system-ui, sans-serif" font-size="13" font-weight="700">' + escapeXml(line) + '</text>')
    })
    if (h.note && h.note.trim()) {
      wrapSvgText(h.note, 34).slice(0, 3).forEach((line, i) => {
        parts.push('<text x="' + (x + 12) + '" y="' + (y + 118 + i * 13) + '" font-family="system-ui, sans-serif" font-size="11">' + escapeXml(line) + '</text>')
      })
    }
    parts.push('</g>')
  })
